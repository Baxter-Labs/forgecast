...
import { JobRunner, ImageJobHandler, EnhanceJobHandler, EditImageJobHandler, CutoutJobHandler, ShortVideoJobHandler, VideoJobHandler, MontageJobHandler, LocalMontageJobHandler, VoiceoverJobHandler, NarrateJobHandler, PresenterJobHandler, LipsyncJobHandler, RetargetJobHandler } from '@forgecast/jobs';
import type { ProjectRepo, AssetRepo, JobRepo, UserRepo, KeyRepo, CharacterRepo, LoraTrainer, StorageDriver, ShortVideoWorker, JobHandler, VideoProvider, VoiceProvider, MontageWorker, Transcriber, PresenterProvider, LipsyncProvider, RetargetProvider, WebsiteReader } from '@forgecast/core';
...
export function buildServices(opts: BuildServicesOptions = {}): Services {
  ...
  const lipsyncProvider: LipsyncProvider = new FalLipsyncProvider({ apiKey: falVideoKey, fetchFn: opts.fetchFn });
  const lipsyncAvailable = lipsyncProvider.isAvailable();
  if (lipsyncAvailable) {
    handlers.push(new LipsyncJobHandler({
  ...
  const retargetProvider: RetargetProvider = new FalWanAnimateProvider({ apiKey: falVideoKey, fetchFn: opts.fetchFn });
  const retargetAvailable = retargetProvider.isAvailable();
  if (retargetAvailable) {
    handlers.push(new RetargetJobHandler({
  ...
  return { imageRegistry, publishers, projects, assets, jobs, users, keys, characters, loraTrainer, storage, runner, ids: { randomId, nowIso }, videoWorker, videoProvider, videoRegistry, videoProviders, montageWorker, montageAvailable, voiceProvider, voiceAvailable, narrateAvailable, transcriber, transcribeAvailable, presenterProvider, presenterAvailable, lipsyncProvider, lipsyncAvailable, retargetProvider, retargetAvailable, websiteReader, insights, insightsAvailable, footage, footageAvailable, fetchFn: opts.fetchFn ?? fetch };
}
