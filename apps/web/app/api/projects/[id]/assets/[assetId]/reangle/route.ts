...
import { reangleAsset } from '@/lib/api';
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string; assetId: string }> }) {
  ...
  const body = await req.json().catch(() => ({})) as { preset?: unknown; instruction?: unknown };
  const r = await reangleAsset(services, id, { assetId, preset: body.preset, instruction: body.instruction });
