...
describe('CloudflareVideoProvider', () => {
  it('is unavailable with neither a binding nor REST creds; available with either', () => {
    ...
    expect(new CloudflareVideoProvider({ runner: { run: async () => ({}) } }).isAvailable()).toBe(true);
    expect(new CloudflareVideoProvider({ accountId: 'a', apiToken: 't' }).isAvailable()).toBe(true);
  });

  it('submits via the async queue API and returns a pollable task id', async () => {
    const run = vi.fn(async () => ({ request_id: 'r1' }));
    const p = new CloudflareVideoProvider({ runner: { run } });
  ...
  it('reports failed on a broken task id and on an explicit failure status', async () => {
    const p = new CloudflareVideoProvider({ runner: { run: async () => ({ status: 'failed' }) } });
